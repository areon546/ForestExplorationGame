...
public class forestGame {
    ...
    public static void enterTheForest() throws IOException {
        ...
        if (choice.equals("A")) {
            ...
            plantsInv = new Plant[MAX_PLANT_INV_LENGTH()];
    ...
    // explores the forest on a daily loop
    //
    public static void exploreTheForest(Save characterSave) throws IOException {
        ...
        while (day < MAX_DAYS && alive) {
            ...
            // TIME loop
            while (time < END_OF_DAY && inForest) {

                // resets the variables that need to be resetted every time increment
                ...
                if (direction.equals("GO HOME") || direction.equals("LEAVE")) {
                    ...
                    // generates an encounter based on
            ...
            saveProgram = saveGamePrompt(); // prompts the user to

            // ends the program is the game is saved
            if (saveProgram == 'y' || saveProgram == 'Q') {
                saveGame(initSave(name, day, plantsInv));
                return;
            } else {
    ...
    public static String chooseDirection(int time) {
        ...
        input = inputString("Which way will you go? ").toUpperCase();
    ...
    public static void printTime(int time) { // TODO figure out why a leading zero changes the time by so much
        ...
        timeOutput = String.format("%n%nThe current time is: %d:%02d", hours, minutes);

        if (minutes >= 60) {
            timeOutput = "%n%nError, time out of bounds. ";
        }
    ...
    public static void saveGame(Save character) throws IOException {
    ...
    public static int MAX_PLANT_INV_LENGTH() {
        return 30;
    }
    ...
    // // TODO update as sir wants
    public static boolean isInt(String possibleInt) {
        try {
            int n = Integer.parseInt(possibleInt);
    ...
    //
    public static boolean isPlantInvFull(Plant[] plantInv) { // TODO fix bug
        boolean isFull = true;
    ...
    public static Plant[] removePlantFromArr(Plant[] plantArr, Animal animal) {
        ...
        while (!swappingValues && plantArrayIndex < highestIndex) {
    ...
    //
    public static String[] saveArrayToCSV(Save[] s) {
        int arrLen = s.length;
    ...
    public static Save csvToSave(String csv) {
        ...
        save = initSave(name, day, plantInv);
...
class Save {
    ...
    static int numberOfSaves = 0; // counter variable for number of Save records
}
